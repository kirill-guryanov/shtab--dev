...
function changeItemsPositionsInFooter() {
  ...
  if (window.innerWidth <= 600) {
    ...
    if (form) {
      ...
      newFooterItem.className = "footer__blocks-item footer__blocks-item--new-item-with-form"
      newFooterItem.appendChild(deletedForm)
      // first item from footer items
      const firstItemWithLogo = document.querySelector(".footer__blocks-item--one")
  ...
  if (window.innerWidth > 600) {
    ...
    let  form
    try {
      formParent = footerItemsList.querySelector(".footer__blocks-item--new-item-with-form")
      form = formParent.querySelector(".footer__right-side-form-content")
    } catch {}
    if (form) {
      // remove form
      ...
      const restoredFormParentElement = footer.querySelector('.footer__right-side-form')
